...
ปุ่ม **REVIEW** ใน FLOW panel (ฝั่งขวา) ไม่เกี่ยวกับ debug mode — เปิดดูไฟล์ recording **ดิบ**ล่าสุดด้วยโปรแกรมเล่นวิดีโอเริ่มต้นของเครื่อง (`xdg-open`) ใช้งานได้หลังติ๊ก **Record video (mp4)** แล้วผ่าน run มาแล้วอย่างน้อยหนึ่งครั้ง (ดูหัวข้อ RECORD — มีไฟล์ `_annotated.mp4` คู่กันด้วยที่ REVIEW ไม่ได้เปิดให้ ต้องเปิดเองถ้าอยากเห็น overlay)
...
```
frame,host_time_iso,t_capture,t_decided,deviation_mm,iris_px,ear,detect,gate,beam_state,reason
```
...
**คอลัมน์ `detect` มีสามค่า ไม่ใช่สอง** — `1` ตาอยู่ในเป้า / `0` เจอหน้าแต่ตาใช้ไม่ได้ (ปิดตาหรือหลุดเป้า) / **ว่าง = หาหน้าไม่เจอเลย** คู่กับ `reason` ที่แยกสาเหตุออกเป็น:

| `reason` | หมายความว่า |
|---|---|
| `no_face` | FaceMesh หาหน้าไม่เจอ — **ไม่รู้ว่าตาอยู่ไหน** |
| `blink` | เจอหน้า แต่ EAR ต่ำกว่าเกณฑ์ = ตาปิด |
| `deviation` | เจอหน้า ตาเปิด แต่อยู่นอก threshold |
| `hold` | ตาพร้อมแล้ว แต่บีมยังพักจากการตัดของตัวเอง (`min_off_s`) |
| `dwell` | ตากลับเข้าเป้าแล้ว แต่ยังนิ่งไม่ครบ (`min_on_target_s`) |
| `kill` / `latched` / `unarmed` | operator หรือสถานะระบบ |

**`no_face` กับ `blink` เคยถูกยุบเป็นอันเดียวกัน** — detection loop เขียน `trigger = result is True` ซึ่งยุบ `None` (หาหน้าไม่เจอ) กับ `False` (ตาปิด) เป็น `False` ก่อนที่ `_gate_reason()` จะได้เห็น branch `no_face` ที่มีอยู่แล้วจึงไปไม่ถึงตลอด **ไม่กระทบความปลอดภัยเลย** เพราะทั้งสองกรณีปิดบีมเหมือนกัน แต่กระทบการวินิจฉัยเต็ม ๆ: `session_20260824_180704` จบด้วย 343 เฟรมที่ถูกบันทึกว่า `blink` ทั้งที่**ตาเปิดโพลง** — ต้องไปไล่ดูวิดีโอทีละเฟรมถึงจะรู้ว่าจริง ๆ คือ FaceMesh หลุดล็อกแล้วหาหน้าไม่เจออีกเลย (`test_no_face_is_not_a_blink` คุมไว้แล้ว)

**คอลัมน์ `ear`** — ค่า eye aspect ratio ดิบที่เทียบกับ `EAR_BLINK_THRESHOLD` เพื่อตัดสิน `blink` เก็บจาก `detection.eye_aspect_ratio()` **ก่อน**เทียบกับ threshold เสมอ (ไม่ใช่แค่ตอนเจอ) เพื่อให้เฟรมที่ถูกตัดสินว่า `blink` มีค่าให้ตรวจสอบย้อนหลังด้วย — ว่างเฉพาะตอน `no_face` (ไม่มีหน้าให้วัด EAR เลย)

เพิ่มเข้ามาเพราะระหว่างไล่ session จริงพบว่า `EAR_BLINK_THRESHOLD = 0.20` (ค่าคงที่จากงานวิจัยหน้าตรงระยะปกติ ไม่ใช่มาโครเฉพาะตาที่หัวเอียง) ตัดสิน `blink` ผิดได้จากสาเหตุที่ไม่ใช่การกระพริบตาจริง — เช่น **เหลือบตาไปมุมสุดทำให้ EAR ตกต่ำกว่าเกณฑ์ทั้งที่ตาเปิดอยู่** (`session_20260824_183359`: blink 134 ครั้ง/นาที ซึ่งเกินอัตรากระพริบตาจริงของมนุษย์ 6-8 เท่า, 58.5% ยาวแค่ 1 เฟรม) ก่อนหน้านี้ต้อง**เอาวิดีโอมา replay ผ่าน FaceMesh เองเพื่อคำนวณ EAR ย้อนหลัง** ซึ่งไม่ reproducible (ผลจากรันซ้ำต่างจากรันสดเพราะ FaceMesh แบบ streaming ขึ้นกับว่าจับล็อกได้ตอนไหน) มีคอลัมน์นี้แล้วอ่านจาก `track.csv` ตรง ๆ ได้เลย
...
── latency budget ────────────────────────────
  ...
  opening leg additionally waits min_off_s = 3 s once the beam is cut,
...
### `plot_beam_overlap.py` — เห็นภาพ ไม่ใช่แค่ตัวเลข

```
python3 plot_beam_overlap.py output/session_<ts>
```

`analyze_latency.py` ตอบเป็นตัวเลขตาราง latency ต่อ transition — ตัวนี้ตอบเป็น**ภาพ**: กราฟ `deviation_mm` (จาก `track.csv`) วางซ้อนบนกราฟ hit rate ของ plane ที่เลือก (default plane 0, จาก `.raw` เดียวกับที่ `analyze_latency.py` ถอด) **บนแกนเวลาเดียวกัน** พร้อมเส้นแนวตั้งทำเครื่องหมายทุกจังหวะ B0 (แดง)/B1 (เขียว) — เห็นด้วยตาว่า "ตาหลุด → บีมตัด → โปรตอนหยุด" เหลื่อมกันแค่ไหน ไม่ต้องรอตัวเลขจากตาราง

**สองนาฬิกาคนละฝั่งถูกเชื่อมด้วยการ fit เชิงเส้นจาก `beam_events.csv`** เอง (`monotonic ≈ a + b × trigger_number`) — ใช้ทุกแถวที่มี ไม่ใช่จุดเดียว เพื่อเฉลี่ย jitter ของ host ทิ้ง (ตัวเลขที่บันทึกในนั้นคือตอนแอปตัดสินใจส่ง `B0`/`B1` ไม่ใช่ timestamp จากฮาร์ดแวร์) — พิมพ์ค่า fit ออกมาด้วยทุกครั้งเทียบกับ `1000/trigger_hz` เชิงทฤษฎี ถ้าคลาดเกิน 1% จะเตือนว่าแกนเวลาของกราฟล่างอาจเชื่อถือไม่ได้

**รันให้เองหลัง STOP เหมือนกัน** — `_auto_plot()` ต่อจาก `_auto_analyze()` บน worker thread เดียวกัน, subprocess แยก, ไม่บล็อกอะไร ผลจะเขียน `plot_overlap.png` ลงในโฟลเดอร์ session เอง ล้มเหลวแบบไม่มีบีม/`.raw` ไม่พอ/`alpide_error` จะไม่ใช่ error แต่ log เป็น `[PLOT] skipped: <เหตุผล>` — เป็นสถานะปกติของทุก run จนกว่าจะมีบีมจริง ไม่ใช่บั๊ก

**ยังไม่เคยมีบีมจริงให้ทดสอบ** — ทดสอบกับ session วันนี้ที่มี `.raw` จริงแล้ว กราฟ deviation กับเส้น B0/B1 ตรงกันแม่นยำ (fit คลาดเคลื่อน 0.01-0.02% จากอัตรา trigger จริง) ส่วนกราฟ hit rate เป็น noise สุ่มไม่มีโครงสร้าง — ตรงตามที่ควรเป็นเมื่อไม่มีโปรตอน (negative control ที่ยืนยันว่าท่อทั้งสายไม่ได้เห็นภาพหลอน) รอบมีบีมจริงถึงจะเห็นว่ากราฟล่างมีขั้นบันไดตรงกับเส้นแดงหรือเปล่า
...
**ได้ไฟล์สองไฟล์ ไม่ใช่ไฟล์เดียว** — `eye_tracking_<ts>.mp4` (ภาพดิบจากกล้องตรงๆ ไม่มีอะไรวาดทับ) กับ **`eye_tracking_<ts>_annotated.mp4`** (เฟรมเดียวกับที่ operator เห็นบนจอ มีวงกลม detection/crosshair เป้า/inset ติดอยู่ด้วย) — `REVIEW` เปิดไฟล์**ดิบ**เท่านั้น (`_rec_path` ชี้ไปที่ path ที่ `start_recording()` รับมา) ถ้าอยากดูว่าตอนนั้น detection ทำงานยังไงต้องเปิด `_annotated.mp4` เอาเอง วาดผ่าน worker thread แยกต่างหาก (queue ที่ไม่บล็อก loop หลัก) เพราะ encode วิดีโอที่สองแบบ synchronous เคยลด `achieved_fps` ไปเกือบครึ่งมาแล้ว (ดูหัวข้อประสิทธิภาพ CPU/next.md)
...
### Min beam-off + Min on-target — กันบีมกระพริบตอนตาอยู่ขอบ threshold
...
**มีสองค่าที่คุมเรื่องนี้ ทำงานคู่กัน — ต้องผ่านทั้งคู่บีมถึงจะเปิด:**

| | นับจาก | คุม |
|---|---|---|
| **`Min beam-off`** (`min_off_s`) | ตอนที่**ตัด** | บีมดับแล้วต้องดับต่ออย่างน้อยเท่านี้ |
| **`Min on-target`** (`min_on_target_s`) | ตอนที่**ตากลับเข้าเป้า** | ตาต้องนิ่งอยู่ในเป้าติดกันเท่านี้ก่อนถึงจะเปิดกลับ |

**ทำไมต้องมีสองตัว** — ถ้ามีแต่ `min_off_s` มันนับจากตอนตัดอย่างเดียว ตาที่กลับเข้ามาที่วินาทีที่ 0.9 ของ hold 1 วินาที จะทำให้บีมเปิดตอนครบ 1.0 วิ ทั้งที่ตาเพิ่งนิ่งได้ 0.1 วิ — แล้วถ้าเฟรมถัดไปตาหลุดพอดี relay ก็ถูกสั่งสับสองครั้งภายใน ~130 ms `min_on_target_s` คือตัวที่ทำให้**การเปิดบีมมาจากหลักฐานว่ามันจะเปิดอยู่ได้** ไม่ใช่มาจากว่า hold บังเอิญหมดตอนเฟรมไหน

**ตัวไหนช้ากว่าตัวนั้นเป็นตัวกำหนด** — ทั้งคู่หน่วงได้เฉพาะขาเปิด ไม่มีค่าไหนของทั้งสองที่ทำให้บีมที่เปิดอยู่ปิดช้าลงได้

ทั้งสองเป็นช่องพิมพ์ใต้ Threshold (ค่าเริ่มต้น `DEFAULT_MIN_OFF_S = 1.0` และ `DEFAULT_MIN_ON_TARGET_S = 1.0` วิ, ตั้ง 0 เพื่อปิดทีละตัว) — เป็นค่าที่คนคำนวณมาจาก run ก่อน ไม่ใช่ค่าที่ต้องลากหาโดยดูภาพสด

**สิ่งที่มันปกป้องคือฮาร์ดแวร์ ไม่ใช่เวลาบีม** ถ้าไม่มีทั้งคู่ relay จะตามสถานะตาทีละเฟรม เล่นซ้ำ session ที่มีพบว่าจะถูกสั่งสับสูงสุดถึง **43 Hz** — นั่นคือสิ่งที่มันมีไว้กัน เพราะฉะนั้นตัวเลขที่ใช้ตัดสินคือ**การสับที่มันกันได้** ไม่ใช่วินาทีบีมที่มันกิน มันคือตัวจำกัดขอบเขต ไม่ใช่ตัว optimize — limiter ที่ไม่เคยต้องทำงานก็ยังทำหน้าที่อยู่

จุดสำคัญสามข้อ:

1. **หน่วงเฉพาะขาเปิด ขาปิดทันทีเสมอ** — ตรรกะทั้งสองตัวอยู่ใน `_gate_open()` จุดเดียวและมีผลกับการ return `True` เท่านั้น เส้นทางที่ปิดบีมทุกเส้นไม่ถูกแตะ เพราะงั้นมันเอนไปทาง "บีมดับ" ได้อย่างเดียว ไม่มีทางทำให้บีมค้างเปิด (`test_beam_gate.py` ตรวจข้อนี้ตรง ๆ: dwell ที่ครบแล้วก็ยังสั่งปิดไม่ได้ และยังแพ้ kill latch)
...
**`track.csv` แยกเหตุให้แล้ว** — `_gate_reason()` เขียน `hold` เมื่อบีมยังพักจากการตัดของตัวเอง และ `dwell` เมื่อตากลับมาแล้วแต่ยังนิ่งไม่พอ สองอย่างนี้หมายถึงคนละเรื่องสำหรับคนอ่าน log

ค่าที่ตั้งบันทึกลง `session.json` (`min_off_s`, `min_on_target_s`) ทุก run

**`analyze_latency.py` ตอบให้เองแล้วว่าค่าที่ตั้งไว้กันอะไรได้** หัวข้อ `── min beam-off: switching the hold rules out ──` ใน `report.txt` (และคีย์ `min_off_sweep` ใน `analysis.json`) เอา `track.csv` ของ run นั้นมา**เล่นซ้ำผ่าน `_gate_open()` ด้วยค่า hold อื่น**:

```
   min_off  cycles   min ON  min OFF   max Hz    duty  beam on
     0.00s      38  0.028s  0.034s     36.2   63.1%   18.87s
     1.00s      26  0.214s  1.007s      4.7   46.7%   13.96s
  →  3.00s      16  0.031s  3.006s     32.8   25.0%    7.49s  ← in force
     5.00s      10  0.101s  5.005s      9.9   14.2%    4.25s
```

แต่ละคอลัมน์มีไว้ทำไม:

- **`min OFF`** — ตัวการันตีเอง คือหน้าที่ของ hold ทั้งหมด **ต้องไม่ต่ำกว่าค่าที่ตั้งไว้เด็ดขาด** (ทุก session ที่วัดมาไม่เคยต่ำกว่าเลย)
- **`min ON`** — ขาที่**ไม่มีอะไรกัน** ขาปิดทันทีเสมอโดยการออกแบบ บีมจึงถูกเปิดแล้วปิดภายในเฟรมเดียวได้ ซึ่งนับเป็นรอบการสับเต็มรอบที่ hold เอื้อมไม่ถึง — เฝ้าดูคอลัมน์นี้ แต่**อย่าไปแก้ด้วยการใส่ "min beam-ON"** เพราะนั่นคือบังคับให้บีมค้างสว่างหลังตาหลุดไปแล้ว ไม่ใช่ทางเลือกที่มีในระบบนี้
- **`max Hz`** — ความถี่สับสูงสุดที่ relay ถูกสั่งในรันนั้น
- **`cycles`** — จำนวนครั้งที่สับทั้งหมด คือสิ่งที่สะสมเป็นการสึกหรอ
- **`duty` / `beam on`** — อยู่ท้ายสุดโดยตั้งใจ นี่คือ**ราคา**ที่จ่าย ไม่ใช่จุดประสงค์

ทำได้เพราะ trigger ต่อเฟรมใน `track.csv` เป็นสถานะของ**ตา** ไม่ได้ขึ้นกับว่าบีมเปิดอยู่หรือเปล่า — เล่นซ้ำด้วย hold คนละค่าจึงให้ผลที่ถูกต้อง ไม่ใช่การวัดใหม่ แต่เป็น run เดิมที่เกิดไปแล้ว เล่นซ้ำกับพารามิเตอร์ที่ตอนนั้นไม่ได้ใช้

**`max Hz` ไม่ได้ดีขึ้นตามค่า hold เสมอไป** — ดูตารางข้างบน 1.00s ให้ 4.7 Hz แต่ 3.00s กลับให้ 32.8 Hz เพราะพอ hold ยาวขึ้น จังหวะที่บีมกลับมาเปิดก็เลื่อนไปตกที่เฟรมอื่น ซึ่งเป็นเรื่องของโชคล้วน ๆ ว่าเฟรมถัดไปตาจะหลุดพอดีหรือเปล่า **ตัวชี้วัดสองตัวไม่ไปทางเดียวกัน:** `cycles` (การสึกหรอสะสม) ดีขึ้นแน่นอนเมื่อ hold ยาวขึ้น แต่ `max Hz` (ความเครียดชั่วขณะ) ไม่แน่ ขึ้นกับว่ากลไกที่ทำให้ shutter พังคืออะไร

**ไม่เล่นซ้ำให้เมื่อ `manual_resume` เปิดอยู่** เพราะขาเปิดขึ้นกับการที่ operator กด RESUME BEAM ซึ่งไม่ได้อยู่ใน log และเล่นซ้ำที่ hold อื่นไม่ได้ — report จะบอกเหตุผลแทนที่จะพิมพ์ตัวเลขที่ผิด

**หัวข้อนี้กับ `── latency budget ──` พิมพ์ออกมาแม้ run นั้นไม่มี detector เลย** (`alpide_error` ไม่ว่าง / ไม่มีไฟล์ `.raw` / decode ไม่ได้) เพราะทั้งสองอันวัดจาก `track.csv` บนนาฬิกาของแอปเอง ไม่ได้อ่านจาก `.raw` — และ run ที่ ALPIDE ไม่ขึ้นคือ run ที่เหลือให้เรียนรู้แค่สองหัวข้อนี้พอดี ส่วนที่อ่านจาก `.raw` ยังหยุดเหมือนเดิมทุกประการ
...
3. **`config.GRAY_CROSSCHECK_ENABLED`** (checkbox "Grayscale cross-check" ในแอป, **default ปิด ตั้งแต่ 2026-08-25**) — ปิดแล้วข้ามการสร้าง FaceMesh ตัวที่สองไปเลย ไม่ใช่แค่ throttle **มีผลตอนเปิด preview ครั้งถัดไปเท่านั้น** เพราะ instance ถูกสร้างครั้งเดียวตอน thread เริ่ม ไม่ได้เช็คทุกเฟรม — วัดแล้ว CPU แทบไม่ลด (258%→255%, throttle ข้อ 2 ทำงานอยู่แล้ว) แต่ **thread ลดจาก 73→54** ซึ่งน่าจะช่วยเรื่อง scheduler contention ที่ทำให้เครื่อง lag มากกว่าตัวเลข %CPU เฉยๆ — เปลี่ยนเป็น default ปิดเพราะมันไม่เคยเป็น redundancy จริง (กล้องตัวเดียวกัน เฟรมเดียวกัน แค่ทำเป็นขาวดำ) และไม่มีอะไรอ่านผลไปตัดสินใจเรื่องบีมเลย ราคาที่จ่ายคือ thread pool ของ FaceMesh ตัวที่สองทำงานอยู่ตลอดที่ preview เปิด ตรงกับอาการเมาส์/คีย์บอร์ดกระตุกที่มีรายงานเข้ามาโดยตรง — เปิดกลับได้จาก checkbox เมื่อต้องการดูค่าเปรียบเทียบจริง ๆ
